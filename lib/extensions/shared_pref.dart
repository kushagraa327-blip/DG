...
Future<bool> setValue(String key, dynamic value, {bool print = true}) async {
  ...
  if (value is String) {
    return await sharedPreferences?.setString(key, value.validate()) ?? false;
  } else if (value is int) {
    return await sharedPreferences?.setInt(key, value.validate()) ?? false;
  } else if (value is bool) {
    return await sharedPreferences?.setBool(key, value.validate()) ?? false;
  } else if (value is double) {
    return await sharedPreferences?.setDouble(key, value.validate()) ?? false;
  } else if (value is Map<String, dynamic>) {
    return await sharedPreferences?.setString(key, jsonEncode(value)) ?? false;
...
Map<String, dynamic> getJSONAsync(String key,
  ...
  if (sharedPreferences?.containsKey(key) == true &&
      sharedPreferences?.getString(key).validate().isNotEmpty == true) {
